...
func NewRouter(cfg config.Config, resolver auth.Resolver, exec handlers.ExecuteService, runtime handlers.RuntimeSnapshotReader) http.Handler {
	...
	mountPublicRoutes(mux, cfg, resolver, exec, runtime)
	mountAdminRoutes(mux, cfg, runtime)
	return mux
}

func NewPublicRouter(cfg config.Config, resolver auth.Resolver, exec handlers.ExecuteService, runtime handlers.RuntimeSnapshotReader) http.Handler {
	mux := http.NewServeMux()
	mountPublicRoutes(mux, cfg, resolver, exec, runtime)
	return mux
}

func NewAdminRouter(cfg config.Config, runtime handlers.RuntimeSnapshotReader) http.Handler {
	mux := http.NewServeMux()
	mountAdminRoutes(mux, cfg, runtime)
	return mux
}

func mountPublicRoutes(mux *http.ServeMux, cfg config.Config, resolver auth.Resolver, exec handlers.ExecuteService, runtime handlers.RuntimeSnapshotReader) {
...
func mountAdminRoutes(mux *http.ServeMux, cfg config.Config, runtime handlers.RuntimeSnapshotReader) {
