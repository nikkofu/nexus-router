...
func (e *ClassifiedError) Error() string {
	return fmt.Sprintf("openai upstream error: status=%d retryable=%t message=%s", e.StatusCode, e.Retryable, e.Message)
}
...
func (a *Adapter) Execute(ctx context.Context, upstream config.ProviderConfig, req canonical.Request) (providers.Result, error) {
	...
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, endpointURL(upstream.BaseURL, req.EndpointKind), bytes.NewReader(body))
	if err != nil {
		return providers.Result{}, err
	}
...
func endpointURL(baseURL string, kind canonical.EndpointKind) string {
	switch kind {
