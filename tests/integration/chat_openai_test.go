...
func TestChatHandlerRejectsUnsupportedPublicImageForms(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			payload: `{
				"model":"openai/gpt-4.1",
				"messages":[{"role":"user","content":[{"type":"input_image","image_url":"https://example.com/cat.png"}]}]
			}`,
			wantSemantics: "invalid_request",
		},
		{
			name: "text item with image field",
			payload: `{
				"model":"openai/gpt-4.1",
				"messages":[{"role":"user","content":[{"type":"text","text":"describe","image_url":{"url":"https://example.com/cat.png"}}]}]
			}`,
			wantSemantics: "invalid_request",
		},
		{
		...
		},
		{
			...
			payload: `{
				"model":"openai/gpt-4.1",
				"messages":[{"role":"user","content":[{"type":"image_url","image_url":{"url":"https://example.com/cat.png","detail":"low"}}]}]
			}`,
